...
const EventEmitter = require('readable-stream').Readable
...
class Pipeline {
  ...
  async getStream () {
    ...
    if (isWritable(this.streams[0])) {
      return duplexify(this.streams[0], lastStream)
    }
  ...
  async _init () {
    ...
    this.streams.forEach((stream, index) => {
      ...
      stream.on('error', cause => {
        ...
        this.context.log.fatal(step.value, cause.message)
      })
    })
